...
class RestorationOFA:
    ...
    def restore_withoutjumps(self, Y, A, Q, COV_CGO, Mean_X, Mean_Y, TProba, IProba):
        ...
        p_rn_dp_y1_to_yn     = np.zeros(shape=(n_r,      N))
        p_rn_dp_y1_to_yN     = np.zeros(shape=(n_r,      N))
        p_rn_rnp1_d_y1_ynp1  = np.zeros(shape=(n_r, n_r))
        # Alpha                = np.zeros(shape=(n_r,      N))   # = Beta Forward
        Beta                 = np.zeros(shape=(n_r,      N))   # = Beta Backward
        p_rnp1_ynp1_dp_rn_yn = np.zeros(shape=(n_r, n_r, N))
        TAB_normalis         = np.zeros(shape=(N))

        E_Xn_dp_rn_y1_to_yn   = np.zeros(shape=(n_x,      n_r, N))
        E2_Xn_dp_rn_y1_to_yn  = np.zeros(shape=(n_x, n_x, n_r, N))
        COV_Xn_dp_rn_y1_to_yn = np.zeros(shape=(n_x, n_x, n_r, N))

        E_R_n   = np.zeros(shape=(N))
        E_R_N   = np.zeros(shape=(N))
        E_X_n   = np.zeros(shape=(n_x,      N))
        E2_X_n  = np.zeros(shape=(n_x, n_x, N))
        COV_X_n = np.zeros(shape=(n_x, n_x, N))
        E_X_N   = np.zeros(shape=(n_x,      N))
        E2_X_N  = np.zeros(shape=(n_x, n_x, N))
        COV_X_N = np.zeros(shape=(n_x, n_x, N))
        ...
        i = 0
        for rn in range(n_r):
            p_rn_dp_y1_to_yn[rn, i] = IProba[rn] * multivariate_normal.pdf(x=Y[:, i], mean=Mean_Y[rn, :], cov=COV_CGO[rn*n_r+rn, s_xz, s_xz])
        ...
        for rn in range(n_r):
            ...
            E_Xn_dp_rn_y1_to_yn  [:,    rn, i] = Mean_X[rn, :] + np.dot(Temp, Y[:, i] - Mean_Y[rn, :])
            COV_Xn_dp_rn_y1_to_yn[:, :, rn, i] = COV_CGO[l, s_0x, s_0x] - np.dot(Temp, COV_CGO[l, s_xz, s_0x])

            Etemp = np.reshape(E_Xn_dp_rn_y1_to_yn[:, rn, i], newshape=(n_x, 1))
            E2_Xn_dp_rn_y1_to_yn [:, :, rn, i] = COV_Xn_dp_rn_y1_to_yn[:, :, rn, i] + np.dot(Etemp, np.transpose(Etemp))
            
            E_X_n  [:,    i] += p_rn_dp_y1_to_yn[rn, i] * E_Xn_dp_rn_y1_to_yn  [:,    rn, i]
            E2_X_n [:, :, i] += p_rn_dp_y1_to_yn[rn, i] * E2_Xn_dp_rn_y1_to_yn [:, :, rn, i]
        ...
        for i in range(1, N):
            ...
            for rn in range(n_r):
                ...
                for rnp1 in range(n_r):
                    ...
                    p_rnp1_ynp1_dp_rn_yn[rn, rnp1, i] = multivariate_normal.pdf(Y[:, i], mean=locmar, cov=scalemar) * TProba[rn, rnp1]

            for rnp1 in range(n_r):
                p_rn_rnp1_d_y1_ynp1[:, rnp1] = p_rnp1_ynp1_dp_rn_yn[:, rnp1, i] * p_rn_dp_y1_to_yn[:, i-1]
                p_rn_dp_y1_to_yn[rnp1, i] = np.sum(p_rnp1_ynp1_dp_rn_yn[:, rnp1, i] * p_rn_dp_y1_to_yn[:, i-1])
            ...
            for rnp1 in range(n_r):
                ...
                for rn in range(n_r):
                    ...
                    p_rn_d_rnp1_y1_ynp1 = p_rn_rnp1_d_y1_ynp1[rn, rnp1] / p_rn_dp_y1_to_yn[rnp1, i]

                    # calcul de l'espérance cond aux sauts
                    Ereshaped = np.reshape(E_Xn_dp_rn_y1_to_yn[:, rn, i-1], newshape=(n_x, 1))
                    E_Xn_dp_rn_y1_to_yn[:, rnp1, i] += p_rn_d_rnp1_y1_ynp1 * np.reshape(H[l, :, :] + np.dot(F[l, :, :], Ereshaped), newshape=(n_x))

                    # calcul de la cov. cond aux sauts
                    T1 = np.dot(np.dot(F[l, :, :], E2_Xn_dp_rn_y1_to_yn[:, :, rn, i-1]), np.transpose(F[l, :, :]))
                    ...
                    E2_Xn_dp_rn_y1_to_yn[:, :, rnp1, i] += p_rn_d_rnp1_y1_ynp1 * (T1+T2+T3+GGT[l, :, :]+T5)

                Etemp = np.reshape(E_Xn_dp_rn_y1_to_yn[:, rnp1, i], newshape=(n_x, 1))
                COV_Xn_dp_rn_y1_to_yn[:, :, rnp1, i] = E2_Xn_dp_rn_y1_to_yn [:, :, rnp1, i] - np.dot(Etemp, np.transpose(Etemp))

            for rnp1 in range(n_r):
                E_X_n  [:,    i] += p_rn_dp_y1_to_yn[rnp1, i] * E_Xn_dp_rn_y1_to_yn  [:,    rnp1, i]
                E2_X_n [:, :, i] += p_rn_dp_y1_to_yn[rnp1, i] * E2_Xn_dp_rn_y1_to_yn [:, :, rnp1, i]
                if np.isnan(E_X_n  [:,    i]):
                    print('i=', i)
        ...
        for i in range(N-2, -1, -1):
            for rn in range(n_r):
                Beta[rn, i] = np.sum(p_rnp1_ynp1_dp_rn_yn[rn, :, i+1] * Beta[:, i+1])
        ...
        return E_X_n, E2_X_n, COV_X_n, E_X_N, E2_X_N, COV_X_N, E_Xn_dp_rn_y1_to_yn, E2_Xn_dp_rn_y1_to_yn, COV_Xn_dp_rn_y1_to_yn, p_rn_dp_y1_to_yN, E_R_n, E_R_N
    ...
    def restore_MultiD(self, Y, A, Q, Mean_X, Mean_Y, TProba, IProba):
        ...
        p_rn_dp_y1_to_yn = np.zeros(shape=(n_r, N))
        # = Beta Backward
        Beta = np.zeros(shape=(n_r, N))
        p_rn_dp_y1_to_yN = np.zeros(shape=(n_r, N))

        E_R_n = np.zeros(shape=(1, N))
        E_R_N = np.zeros(shape=(1, N))
        ...
        E_Xn_dp_rn_y1_to_yn   = np.zeros(shape=(n_r, n_x, N))
        Cov_Xn_dp_rn_y1_to_yn = np.zeros(shape=(n_r, n_x, n_x, N))
        E_X_n  = np.zeros(shape=(n_x, N))
        Cov_Xn = np.zeros(shape=(n_x, n_x, N))
        E_X_N  = np.zeros(shape=(n_x, N))
        Cov_XN = np.zeros(shape=(n_x, n_x, N))
        ...
        for i in range(n_r):
            E_Xn_dp_rn_y1_to_yn  [i, :, 0]    = np.transpose(Mean_X)
            Cov_Xn_dp_rn_y1_to_yn[i, :, :, 0] = np.ones(n_x)
            E_X_n[:, 0] += p_rn_dp_y1_to_yn[i, 0]*E_Xn_dp_rn_y1_to_yn[i, :, 0]
            Cov_Xn[:, :, 0] += p_rn_dp_y1_to_yn[i, 0] * Cov_Xn_dp_rn_y1_to_yn[i, :, :, 0]

        N_xy = np.zeros(shape=(n_r_2, n_z, N))
        ...
        for i in range(1, N):
            for l in range(n_r_2):
                ...
                p_temp[l]                  = p_rnp1_yn1_dp_rn_yn[i-1, l] * p_rn_dp_y1_to_yn[l//n_r, i-1]
            ...
            for l in range(n_r_2):
                N_xy[l, :, i] = np.hstack(((Mean_X[l%n_r, :] - np.dot(A[l, s_0x, s_0x], Mean_X[l//n_r, :])- np.dot(A[l, s_0x, s_xz], Mean_Y[l//n_r, :])), (Mean_Y[l%n_r, :]-np.dot(A[l, s_xz, s_0x], Mean_X[l//n_r, :])-np.dot(A[l, s_xz, s_xz], Mean_Y[l//n_r, :]))))
            ...
            for l in range(n_r_2):
                ...
                H[l, :] = np.dot(J[l, :, :], (Y[:, i] - np.dot(A[l, s_xz, s_xz], Y[:, i-1]) - N_xy[l, s_xz, i])) + np.dot(A[l, s_0x, s_xz], Y[:, i-1])+N_xy[l, s_0x, i]
            ...
            for j in range(n_r):
                l, q = 0, 0
                while j+l < n_r_2:
                    E_Xn_dp_rn_y1_to_yn[j, :, i] += p_rn_dp_rn1_y1_to_yn1[j+l] * (np.dot(F[j+l, :, :], E_Xn_dp_rn_y1_to_yn[q, :, i-1]) + H[j+l, :])

                    Cov_Xn_dp_rn_y1_to_yn[j, :, :, i] += p_rn_dp_rn1_y1_to_yn1[j+l]*(np.dot(np.dot(F[j+l, :, :], Cov_Xn_dp_rn_y1_to_yn[q, :, :, i-1]), np.transpose(F[j+l, :, :])) + \
                            np.dot(np.dot(F[j+l, :, :], E_Xn_dp_rn_y1_to_yn[q, :, i-1]), np.transpose(H[j+l, :])) + np.dot(np.dot(H[j+l, :], np.transpose(E_Xn_dp_rn_y1_to_yn[q, :, i-1])), \
        ...
        for i in range(N-2, -1, -1):
            for l in range(n_r_2):
                j = l//n_r
                k = l%n_r
                p_temp[l]  = p_rnp1_yn1_dp_rn_yn[i, l] * Beta[k, i+1]
                p_temp1[l] = p_rnp1_yn1_dp_rn_yn[i, l] * p_rn_dp_y1_to_yn[j, i]
