...
def IntegDouble_Predictor(EPS, STEPS, proba, tab_E, n_z, Rcentres):
    ...
    for indrnp1, rnp1 in enumerate(Rcentres):
        pR[indrnp1, :] = proba.getindr(indrn, indrnp1+1) * np.reshape(tab_E.getindr(indrn, indrnp1+1), newshape=(n_z))
    integ = np.mean(pR) + proba.getindr(0, 0)*tab_E.getindr(0, 0) + proba.getindr(0, STEPS+1)*tab_E.getindr(0, STEPS+1)

    #### pour r1==1.
    rn, indrn = 1., STEPS+1
    for indrnp1, rnp1 in enumerate(Rcentres):
        pR[indrnp1, :] = proba.getindr(indrn, indrnp1+1) * np.reshape(tab_E.getindr(indrn, indrnp1+1), newshape=(n_z))
    integ += np.mean(pR) + proba.getindr(STEPS+1, 0)*tab_E.getindr(STEPS+1, 0) + proba.getindr(STEPS+1, STEPS+1)*tab_E.getindr(STEPS+1, STEPS+1)

    # #### La surface à l'intérieur
    for indrn, rn in enumerate(Rcentres):
        for indrnp1, rnp1 in enumerate(Rcentres):
            pR[indrnp1, :] = proba.getindr(indrn+1, indrnp1+1) * np.reshape(tab_E.getindr(indrn+1, indrnp1+1), newshape=(n_z))
        integ += np.mean(pR) + proba.getindr(indrn+1, 0)*tab_E.getindr(indrn+1, 0) + proba.getindr(indrn+1, STEPS+1)*tab_E.getindr(indrn+1, STEPS+1)
...
class RestorationOFAFuzzy:
    ...
    def restore_Fuzzy1D(self, Y, filt=True, smooth=False, predic=False):
        ...
        p_rnp1_dp_rn = Loi1DDiscreteFuzzy_TMC(EPS, self.__STEPS, self.__interpolation, self.__Rcentres)
        ...
        if predic:
            p_rnp1_dp_rn.setProbaPredict_1(self.__FS.probaR)
            flevel_max_pred, proba_max_pred = p_rnp1_dp_rn.fuzzyMPM_1D()
            E_R_np1[np1] = flevel_max_pred
        if smooth:
        ...
        if predic:
            ...
            VAR_Z_np1[np1, :, :] = 0. # pas de prediction
        ...
        for np1 in range(1, N):
            ...
            if filt:
                ...
                E_R_n[np1] = flevel_max_filt
            if predic:
                p_rnp1_dp_rn.setProbaPredict(self.__FS.probaR2CondR1, E_R_np1[np1-1])
                flevel_max_pred, proba_max_pred = p_rnp1_dp_rn.fuzzyMPM_1D()
                E_R_np1[np1] = flevel_max_pred
            if smooth:
                flevel_max_smoo, proba_max_smoo = tab_p_rn_dp_y1_to_yN[np1].fuzzyMPM_1D()
            ...
            if predic:
                ...
                B = IntegDouble_Predictor(EPS, self.__STEPS, tab_p_rnnp1_dp_y1_to_yn[np1], VAR_Znp1_dp2, self.__n_z, self.__Rcentres)
                VAR_Z_np1[np1, :, :] = B #np.reshape(B, newshape=(self.__n_z, self.__n_z))
                if self.__verbose >= 3:
